...
including O2 start, included travel time, and the terminal `<= 35 min` no-air-break
exception at `20 fsw`.
...
- The `30 -> 20` ascent time is included in the `20 fsw` O2 stop time.
- No five-minute air break is required during the `20 fsw` stop because the final
  oxygen period is `35 min` or less.
...
- Failure: restarting the `20 fsw` obligation incorrectly instead of honoring O2
  continuity rules
- Failure: inserting an air break into the final `35 min` `20 fsw` O2 period
